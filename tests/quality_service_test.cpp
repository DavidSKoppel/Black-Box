...
TEST_F(QualityServiceTest, DiscountBoundaries) {
    ...
    EXPECT_EQ(service.calculateDiscount({100, false, "", false, 12}), 0);   // was 10 → fixed
    EXPECT_EQ(service.calculateDiscount({500, false, "", false, 12}), 20);
    EXPECT_EQ(service.calculateDiscount({1000, false, "", false, 12}), 30); // was 20 → fixed
...
TEST_F(QualityServiceTest, BookingRules) {
    ...
    EXPECT_TRUE(service.canBookSeats({6, false, 10, false}));  // fixed
...
TEST_F(QualityServiceTest, UsernameRules) {
    ...
    EXPECT_EQ(service.formatUsername("   "), "");
}
